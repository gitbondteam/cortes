...
MAX_DIMENSION = 1600
SLUGS = ("tomahawk", "corte-americano", "aranita")


def source_paths():
    catalog = (ROOT / "src/data/products.ts").read_text()
    entries = dict(re.findall(r"slug: '([^']+)'[\s\S]*?image: '([^']+)'", catalog))
    return {slug: ROOT / "public" / entries[slug].lstrip("/") for slug in SLUGS}
...
def main():
    ...
    report = []
    for slug, source in source_paths().items():
        ...
        output = OUTPUT / f"{slug}-white-bg.png"
